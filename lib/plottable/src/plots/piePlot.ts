...
export module Plots {
  export class Pie extends Plot {
     ...
     **/
    public entitiesAt(queryPoint: Point, offsetAngle: number = 0) {
      let center = { x: this.width() / 2, y: this.height() / 2 };
      let adjustedQueryPoint = { x: queryPoint.x - center.x, y: queryPoint.y - center.y };
      let index = this._sliceIndexForPoint(adjustedQueryPoint, offsetAngle);
    ...
    private _generateStrokeDrawSteps(): Drawers.DrawStep[] {
      return this._generateDrawSteps();
    }

    protected _generateDrawSteps(): Drawers.DrawStep[] {
      let attrToProjector = this._generateAttrToProjector();
      return [{attrToProjector: attrToProjector, animator: new Animators.Arc(this._startAngles, this._endAngles)}];
    }

    private _sliceIndexForPoint(p: Point, offsetAngle: number) {
