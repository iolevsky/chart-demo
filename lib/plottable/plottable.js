    ...
    (function (Utils) {
        var ClientToSVGTranslator = (function () {
            ...
            ClientToSVGTranslator.prototype.computePosition = function (clientX, clientY) {
                ...
                var halfClientWidth = this._measureRect.ownerSVGElement.clientWidth / 2;
                var halfClientHeight = this._measureRect.ownerSVGElement.clientHeight / 2;
                // this supports two possible cases for origin location: either top left corner (0,0)
                // or center of the SVG element (halfCientWidth, halfClientHeight)
                origin.x = origin.x >= halfClientWidth ? origin.x - halfClientWidth : origin.x;
                origin.y = origin.y >= halfClientHeight ? origin.y - halfClientHeight : origin.y;
                var scaledPosition = {
...
(function (Plottable) {
    var Drawers;
    (function (Drawers) {
        var ArcAll = (function (_super) {
            __extends(ArcAll, _super);
            function ArcAll(dataset) {
                _super.call(this, dataset);
...
(function (Plottable) {
    ...
    var Component = (function () {
        function Component() {
            ...
            this._fixedWidth = 0; // Requested fixed width and height. Used if non-zero and smaller than available space.
            this._fixedHeight = 0;
        ...
        Component.prototype.requestedSpace = function (availableWidth, availableHeight) {
            return {
                minWidth: this._fixedWidth,
                minHeight: this._fixedHeight
            };
        };
        /**
        ...
        Component.prototype.computeLayout = function (origin, availableWidth, availableHeight) {
            ...
            if (origin == null || availableWidth == null || availableHeight == null) {
                if (this._element == null) {
                    ...
                    //TODO: check if viewbox is set, and use element size if not
                    var svg = this._rootSVG[0][0];
                    availableWidth = svg.viewBox.baseVal.width;
                    availableHeight = svg.viewBox.baseVal.height;
                }
                else {
            ...
            this._origin = {
                x: origin.x - this.width() * xAlignProportion,
                y: origin.y - this.height() * yAlignProportion
            };
        ...
        Component.prototype.getRootSVG = function () {
            if (this._rootSVG) {
                return this._rootSVG[0][0];
            }
            return this.parent().getRootSVG();
        };
        /**
        ...
        Component.prototype.fixedDimensions = function (width, height) {
         ...
         * IO: Since 'origin' here implies top left corner of the component,
         *     in case of SVG origin not being top left of element requires
         *     adjustment based on viewbox settings
         * @return {Point}
         */
        Component.prototype.origin = function () {
            var svg = this.getRootSVG();
            if (svg.viewBox.baseVal.x !== 0 || svg.viewBox.baseVal.y !== 0) {
                return {
                    x: Math.abs(svg.viewBox.baseVal.x) - this._width / 2,
                    y: Math.abs(svg.viewBox.baseVal.y) - this._width / 2
                };
            }
            return {
                x: this._origin.x,
...
(function (Plottable) {
    ...
    (function (Components) {
        var Shape = (function (_super) {
            ...
            Shape.prototype.checkPointCollision = function (point) {
                return false;
            };
            Shape.prototype.renderImmediately = function () {
                var drawSteps = [{ attrToProjector: this._generateAttrToProjector(), animator: this._createAnimator() }];
                this._drawer.draw([{}], drawSteps);
                return this;
            };
            Shape.prototype._setup = function () {
            ...
            Shape.prototype._createDrawer = function (config) {
                return new Plottable.Drawer(new Plottable.Dataset([config]));
            };
            Shape.prototype._createAnimator = function () {
                return new Plottable.Animators.Null();
            };
            Shape.prototype._generateAttrToProjector = function () {
                var h = {};
...
(function (Plottable) {
    ...
    (function (Components) {
        var Arc = (function (_super) {
            __extends(Arc, _super);
            function Arc(innerRadius, outerRadius, startAngle, endAngle, config) {
            ...
            Arc.prototype.setSize = function (innerRadius, outerRadius) {
                this._innerRadius = innerRadius;
                this._outerRadius = outerRadius;
                this._updateArcLayout();
                return this;
            };
            Arc.prototype.addDataset = function (dataset) {
                this._dataset = dataset;
                this._updateArcLayout();
                return this;
            };
            Arc.prototype.arcLabel = function (callback) {
                this._arcLabel = callback;
                return this;
            };
            Arc.prototype.computeLayout = function (origin, availableWidth, availableHeight) {
                _super.prototype.computeLayout.call(this, origin, availableWidth, availableHeight);
                //this._renderArea.attr("transform", "translate(" + this._outerRadius + "," + this._outerRadius + ")");
                return this;
            };
            Arc.prototype.checkPointCollision = function (point) {
                var distance = Math.sqrt(Math.pow(this._outerRadius - point.x, 2) + Math.pow(this._outerRadius - point.y, 2));
                return distance <= this._outerRadius;
            };
            Arc.prototype.renderImmediately = function () {
                var drawSteps = [{ attrToProjector: this._generateAttrToProjector(), animator: this._createAnimator() }];
            ...
            Arc.prototype._createDrawer = function (config) {
                return new Plottable.Drawers.ArcAll(this._dataset);
            };
            Arc.prototype._createAnimator = function () {
                return new Plottable.Animators.Arc(this._startAngles, this._endAngles);
            };
            Arc.prototype._sizeFromOffer = function (availableWidth, availableHeight) {
                return {
                    width: this._outerRadius * 2,
                    height: this._outerRadius * 2
                };
            };
            Arc.prototype._updateArcLayout = function () {
                var _this = this;
                ...
                this._config["d"] = function (datum, index) {
                    return d3.svg.arc()
                        .innerRadius(_this._innerRadius)
                        .outerRadius(_this._outerRadius)
                        .startAngle(_this._startAngles[index])(datum, index);
                };
                var shapeId = this._id;
                this._config["id"] = function (datum, index) {
                    return shapeId + "arc" + index;
                };
            };
            Arc.prototype._drawLabels = function () {
                var _this = this;
                var labelArea = this._renderArea.append("g").classed("label-area", true);
                var defs = labelArea.append("defs");
                var data = this._dataset.data();
                var clipArc = function (datum, index) {
                    return d3.svg.arc()
                        .innerRadius(_this._innerRadius)
                        .outerRadius(_this._outerRadius)
                ...
                data.forEach(function (datum, datumIndex) {
                    var value = _this._arcLabel(datum, datumIndex, _this._dataset);
                    var arcId = _this._id + "arc" + String(datumIndex);
                    var clipId = _this._id + "textclip" + String(datumIndex);
                    var textPathId = _this._id + "textpath" + String(datumIndex);
                    defs.append("path")
                        .attr("id", textPathId)
                    ...
                    var text = labelArea.append("text")
...
(function (Plottable) {
    ...
    (function (Components) {
        var Circle = (function (_super) {
            ...
            Circle.prototype.computeLayout = function (origin, availableWidth, availableHeight) {
                _super.prototype.computeLayout.call(this, origin, availableWidth, availableHeight);
                this._renderArea.attr("transform", "translate(" + this._config.r + "," + this._config.r + ")");
                return this;
            };
            Circle.prototype.checkPointCollision = function (point) {
                var distance = Math.sqrt(Math.pow(this._config.r - point.x, 2) + Math.pow(this._config.r - point.y, 2));
                return distance <= this._config.r;
            };
            Circle.prototype._createDrawer = function (config) {
                return new Plottable.Drawers.Circle(new Plottable.Dataset([config]));
            };
            Circle.prototype._sizeFromOffer = function (availableWidth, availableHeight) {
                return {
    ...
    (function (Plots) {
        var Pie = (function (_super) {
            ...
            Pie.prototype.computeLayout = function (origin, availableWidth, availableHeight) {
                ...
                if (this.innerRadius().scale != null && this.innerRadius().scale.range().toString() === "0,1") {
                    this.innerRadius().scale.range([0, radiusLimit]);
                }
                if (this.outerRadius().scale != null && this.outerRadius().scale.range().toString() === "0,1") {
             ...
             * @param {PlotEntity[]}
             */
            /** IO: offsetAngle is an optional rotation angle of the entire wheel.
             *     It is required in order to get accurate detection pie wedges based
             *     on current mouse location when pie is rotated about its center.
             **/
            Pie.prototype.entitiesAt = function (queryPoint, offsetAngle) {
                if (offsetAngle === void 0) { offsetAngle = 0; }
                var center = { x: this.width() / 2, y: this.height() / 2 };
                var adjustedQueryPoint = { x: queryPoint.x - center.x, y: queryPoint.y - center.y };
                var index = this._sliceIndexForPoint(adjustedQueryPoint, offsetAngle);
            ...
            Pie.prototype._generateStrokeDrawSteps = function () {
                return this._generateDrawSteps();
            };
            Pie.prototype._generateDrawSteps = function () {
                var attrToProjector = this._generateAttrToProjector();
                return [{ attrToProjector: attrToProjector, animator: new Plottable.Animators.Arc(this._startAngles, this._endAngles) }];
            };
            Pie.prototype._sliceIndexForPoint = function (p, offsetAngle) {
                ...
                // compensate for rotation angle of the pie chart
                pointAngle = pointAngle - offsetAngle * Math.PI / 180;
                if (pointAngle < 0) {
                    pointAngle = Math.PI * 2 + pointAngle;
                }
                var index;
...
(function (Plottable) {
    ...
    (function (Animators) {
        ...
        var Arc = (function (_super) {
             ...
             * @constructor
             */
            function Arc(startAngles, endAngles) {
                _super.call(this);
                this._endAngles = endAngles;
                this._startAngles = startAngles;
            }
            //arcanimate
            Arc.prototype.animate = function (selection, attrToAppliedProjector) {
                var _this = this;
                var numberOfSteps = selection[0].length;
                var adjustedIterativeDelay = this._getAdjustedIterativeDelay(numberOfSteps);
                return selection.transition()
                    ...
                    .delay(function (d, i) { return _this.startDelay() + adjustedIterativeDelay * i; })
                    .attr(attrToAppliedProjector)
                    .attrTween("d", function (d, index) { return _this._pathTween(d, index, attrToAppliedProjector["d"]); });
            };
            Arc.prototype._pathTween = function (d, index, dProjector) {
